      ...
      <div style="background:#fff;padding:15px;height:100%;">
        <!-- 按钮组 -->
          <div v-if="apiButtonData.length>0" style="height:40px;line-height:40px;width:100%;background-color:#fff;margin-top:2px;margin-bottom:10px;">
            <el-button v-for="item in apiButtonData" :class="item.fieldEnName" :key="item.id" :type="index===0?item.type:''" :size="item.width" @click="handleAction(item.fieldEnName,item.id)">
              {{item.name}}
              <!-- 简单导出 -->
