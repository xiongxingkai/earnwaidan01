<template>
  <div class="tags-view-container">
    <scroll-pane class="tags-view-wrapper" ref="scrollPane" :style="{'height': height+'px'}">
      <router-link ref="tag" class="tags-view-item" :class="isActive(tag)?'active':''" v-for="tag in Array.from(visitedViews)" :to="tag.path" :key="tag.path" @contextmenu.prevent.native="openMenu(tag,$event)">
        ...
        <span class="el-icon-close" @click.prevent.stop="closeSelectedTag(tag)" v-if="$store.getters.cachedViews[0]!=='自助录入'"></span>
      </router-link>
    </scroll-pane>
...
export default {
  ...
  mounted() {
    this.addViewTags()
    this.$store.commit('SET_TAGSVIEWHEIGHT', this.height)
  },
  methods: {
    generateTitle,
    generateRoute() {
    ...
    closeSelectedTag(view) {
      this.$store.dispatch('delVisitedViews', view).then(views => {
        if (this.isActive(view)) {
          const latesView = views.slice(-1)[0]
          if (latesView) {
            this.$router.push(latesView.path)
          } else {
            this.$router.push('/')
          }
        }
