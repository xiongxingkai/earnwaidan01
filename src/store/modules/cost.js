const cost = {
  ...
  state: {
    ...
    processId: '', // 流程处理id
    taskId: '', // 流程任务id
    selectTabIndex: '0', // 选择TAB下标
    selectTabName: 'myToDo', // 选择TAB下标名
    applicationBudget: false, // 驳回按钮
    buttonClickNum: null // 点击按钮编号
  },
  mutations: {
    // 详情id
    ...
    // 选择Tab下标
    SET_SelectTabIndex: (state, selectTabIndex) => {
      state.selectTabIndex = selectTabIndex
    },
    // 选择Tab下标名称
    SET_SelectTabName: (state, selectTabName) => {
      state.selectTabName = selectTabName
    },
    // 驳回按钮控制
    SET_ApplicationBudget: (state, applicationBudget) => {
      state.applicationBudget = applicationBudget
    },
    // 按钮编号
    SET_ButtonClickNum: (state, buttonClickNum) => {
      state.buttonClickNum = buttonClickNum
    },
    // 控制跳转
    TOGGLE_STATUS: (state, status) => {
      state.hcostStatus = status
    },
    // 办理id
  ...
  actions: {
    ...
    // 选择Tab下标
    setSelectTabIndex({ commit }, selectTabIndex) {
      commit('SET_SelectTabIndex', selectTabIndex)
    },
    // 选择Tab下标名称
    setSelectTabName({ commit }, selectTabName) {
      commit('SET_SelectTabName', selectTabName)
    },
    // 驳回按钮控制
    setApplicationBudget({ commit }, applicationBudget) {
      commit('SET_ApplicationBudget', applicationBudget)
    },
    // 按钮编号
    setButtonClickNum({ commit }, buttonClickNum) {
      commit('SET_ButtonClickNum', buttonClickNum)
    },
    // 控制跳转
